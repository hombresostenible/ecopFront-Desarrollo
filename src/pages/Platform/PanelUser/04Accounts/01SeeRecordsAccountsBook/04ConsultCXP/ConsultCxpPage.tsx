...
import { getAccountsPayable, getAccountsPayableByBranch } from '../../../../../../redux/User/indicator/finantialIndicators/actions.ts';
...
function ConsultCxpPage() {
    ...
    const { accountsPayable, totalRegisters } = useSelector((state: RootState) => state.finantialIndicators);
    ...
    useEffect(() => {
        const fetchData = async (page: number, limit: number) => {
            try {
                await dispatch(getAccountsPayable(token, page, limit));
            } catch (error) {
    ...
    useEffect(() => {
        if (token) {
            if (selectedBranch) {
                const fetchData = async (page: number, limit: number) => {
                    try {
                        await dispatch(getAccountsPayableByBranch(selectedBranch, token, page, limit));
                    } catch (error) {
                        throw new Error('Error al traer los registros');
                    }
                };
    ...
    const handleFilter = () => {
        ...
        const filtered = Array.isArray(accountsPayable)
            ? accountsPayable.filter((accountsPayable) => {
                const transactionDate = format(new Date(accountsPayable.transactionDate), 'yyyy-MM-dd');
                return (
                    transactionDate >= formattedStartDate &&
                    transactionDate <= formattedEndDate &&
                    (!selectedBranch || accountsPayable.branchId === selectedBranch)
                );
            }) : [];
            ...
            <div className='d-flex'>
                ...
                <div className={`${styles.container} d-flex flex-column align-items-center justify-content-between overflow-hidden overflow-y-auto`}>
                    <div className={`${styles.container__Component} px-5 overflow-hidden overflow-y-auto`}>
                        <h1 className={`${styles.title} mb-4 mt-4 mx-auto`}>Ver todas cuentas por pagar</h1>
                            ...
                            <table className="table">
                                ...
                                <tbody className={`${styles.container__Body} `}>
                                    {Array.isArray(transactionsToShow) && transactionsToShow.length > 0 ? (
                                        transactionsToShow.map((accountsPayable) => (
                                            <tr key={accountsPayable.id} className={`${styles.container__Info} d-flex align-items-center justify-content-between`}>
                                                {selectedColumns.includes('Fecha de transacción') && (
                                                    <td className={`${styles.transaction__Date} pt-0 pb-0 px-2 d-flex align-items-center justify-content-center overflow-hidden`}>
                                                        <span className={`${styles.text__Ellipsis} overflow-hidden`}>{new Date(accountsPayable.transactionDate).toLocaleDateString('en-GB')}</span>
                                                    </td>
                                                )}
                                                {selectedColumns.includes('Sede') && (
                                                    <td className={`${styles.branch} pt-0 pb-0 px-2 d-flex align-items-center justify-content-center overflow-hidden`}>
                                                        <span>
                                                            {Array.isArray(branches) && branches.map((branch, index) => (
                                                                accountsPayable.branchId === branch.id && (
                                                ...
                                                {selectedColumns.includes('Valor total') && (
                                                    <td className={`${styles.total__Value} pt-0 pb-0 px-2 d-flex align-items-center justify-content-center overflow-hidden`}>
                                                        <span className={`${styles.text__Ellipsis} overflow-hidden`}>{accountsPayable.currentBalance? `$ ${formatNumber(accountsPayable.currentBalance)}` : 'N/A'}</span>
                                                    </td>
                                                )}
                                                {selectedColumns.includes('Deudor') && (
