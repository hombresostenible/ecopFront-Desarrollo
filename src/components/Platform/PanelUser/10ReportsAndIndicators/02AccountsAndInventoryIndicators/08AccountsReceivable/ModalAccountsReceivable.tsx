...
import { getAccountsReceivable } from '../../../../../../redux/User/indicator/finantialIndicators/actions';
...
function ModalAccountsReceivable () {
    ...
    const [filteredRegisters, setFilteredRegisters] = useState(accountsReceivable);
    ...
    useEffect(() => {
        if (Array.isArray(accountsReceivable)) {
            if (selectedBranch === 'Todas') {
                setFilteredRegisters(accountsReceivable);
            } else {
                setFilteredRegisters(accountsReceivable.filter((sale) => sale.branchId === selectedBranch));
            }
        } else setFilteredRegisters([]);
    }, [selectedBranch, accountsReceivable]);
    ...
    return (
        <div className="p-3 text-center m-auto border">
            <h2 className="mb-3 text-primary-emphasis text-start">Cuentas por cobrar</h2>
            ...
            <div className="mt-4 col-12"> 
                {Array.isArray(filteredRegisters) && filteredRegisters.length > 0 ? (
                    <table className="table table-bordered table-striped">
                        ...
                        <tbody>
                            {filteredRegisters.map((account: IAccountsReceivable) => (
                                <tr key={account.id}>
                                    <td>
