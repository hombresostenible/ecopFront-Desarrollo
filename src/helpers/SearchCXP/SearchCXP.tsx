...
import { getAccountsPayableByBranchPaginated } from '../../redux/User/indicator/finantialIndicators/actions';
...
function SearchCXP({ token, selectedBranch, onCXPSelect }: SearchCXPProps) {
    ...
    useEffect(() => {
        if (selectedBranch) dispatch(getAccountsPayableByBranchPaginated(selectedBranch, token));
    }, [ token ]);
